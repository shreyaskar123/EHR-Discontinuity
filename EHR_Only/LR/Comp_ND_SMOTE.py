...
def scores(X,y):
    ...
    actual = y
    #file = open('comp_smote_lr_ehr.out', 'a')
    print(accuracy_score(actual,pred),file = open('comp_smote_lr_ehr.out', 'a'))
    #print(f1_score(actual,pred),file = open('comp_smote_lr_ehr.out', 'a'))
    print(f1_score(actual,pred, average = 'macro'),file = open('comp_smote_lr_ehr.out', 'a'))
    print(fbeta_score(actual,pred, average = 'macro', beta = 2),file = open('comp_smote_lr_ehr.out', 'a'))
    print(roc_auc_score(actual, best_clf.predict_proba(X)[:,1]),file = open('comp_smote_lr_ehr.out', 'a'))
    print(log_loss(actual,best_clf.predict_proba(X)[:,1]),file = open('comp_smote_lr_ehr.out', 'a'))
...
def cross_val(X,y,Or_X, Or_y):
    ...
    print(np.mean(accuracy),file = open('comp_smote_lr_ehr.out', 'a'))
    print(np.mean(f1),file = open('comp_smote_lr_ehr.out', 'a'))
    print(np.mean(f2),file = open('comp_smote_lr_ehr.out', 'a'))
    print(np.mean(auc),file = open('comp_smote_lr_ehr.out', 'a'))
    print(np.mean(log_loss),file = open('comp_smote_lr_ehr.out', 'a'))
...
print("Gpop",file = open('comp_smote_lr_ehr.out', 'a'))

from imblearn.over_sampling import SMOTE
sm = SMOTE(random_state = 42)
co_train_gpop_sm,out_train_comp_gpop_sm = sm.fit_resample(co_train_gpop,out_train_comp_gpop)

best_clf = lr(co_train_gpop_sm, out_train_comp_gpop_sm)
...
print("",file = open('comp_smote_lr_ehr.out', 'a'))
...
print("Low",file = open('comp_smote_lr_ehr.out', 'a'))
from imblearn.over_sampling import SMOTE
sm = SMOTE(random_state = 42)
co_train_low_sm,out_train_comp_low_sm = sm.fit_resample(co_train_low,out_train_comp_low)

best_clf = lr(co_train_low_sm, out_train_comp_low_sm)
...
print("High",file = open('comp_smote_lr_ehr.out', 'a'))
from imblearn.over_sampling import SMOTE
sm = SMOTE(random_state = 42)
co_train_high_sm,out_train_comp_high_sm = sm.fit_resample(co_train_high,out_train_comp_high)

best_clf = lr(co_train_high_sm, out_train_comp_high_sm)
