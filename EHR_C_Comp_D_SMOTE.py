...
def scores(X,y):
    ...
    actual = y
    print(accuracy_score(actual,pred), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(f1_score(actual,pred), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(fbeta_score(actual,pred, average = 'macro', beta = 2), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(roc_auc_score(actual, best_clf.predict_proba(X)[:,1]), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(log_loss(actual,best_clf.predict_proba(X)[:,1]), file = open('comp_d_smote_rf_ehrc.out', 'a'))
...
def cross_val(X,y):
    ...
    print(np.mean(accuracy), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(np.mean(f1), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(np.mean(f2), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(np.mean(auc), file = open('comp_d_smote_rf_ehrc.out', 'a'))
    print(np.mean(log_loss), file = open('comp_d_smote_rf_ehrc.out', 'a'))

# # General Population
# 

# In[8]:
print("gpop Sep 3", file = open('comp_d_smote_rf_ehrc.out', 'a'))


from imblearn.over_sampling import SMOTE
sm = SMOTE(random_state = 42)
co_train_gpop_sm,out_train_comp_gpop_sm = sm.fit_resample(co_train_gpop,out_train_comp_gpop)
print("", file = open('comp_d_smote_rf_ehrc.out', 'a'))
...
print("Low", file = open('comp_d_smote_rf_ehrc.out', 'a'))
...
print("High", file = open('comp_d_smote_rf_ehrc.out', 'a'))
