...
def scores(X,y):
    ...
    actual = y
    print(accuracy_score(actual,pred), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(f1_score(actual,pred), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(fbeta_score(actual,pred, average = 'macro', beta = 2), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(roc_auc_score(actual, best_clf.predict_proba(X)[:,1]), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(log_loss(actual,best_clf.predict_proba(X)[:,1]), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
...
def cross_val(X,y):
    ...
    print(np.mean(accuracy), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(np.mean(f1), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(np.mean(f2), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(np.mean(auc), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
    print(np.mean(log_loss), file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
...
best_clf = xgBoost(co_train_gpop_FAMD, out_train_comp_gpop)

print("Train gpop", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))
cross_val(co_train_gpop_FAMD, out_train_comp_gpop)

print()

print("Test gpop", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))

scores(co_validation_gpop_FAMD, out_validation_comp_gpop)
...
best_clf = xgBoost(co_train_high_FAMD, out_train_comp_high)

print("Train high", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))

cross_val(co_train_high_FAMD, out_train_comp_high)

print()

print("Test high", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))

scores(co_validation_high_FAMD, out_validation_comp_high)
...
best_clf = xgBoost(co_train_low_FAMD, out_train_comp_low)

print("Train low", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))

cross_val(co_train_low_FAMD, out_train_comp_low)

print()

print("Test low", file = open('comp_nd_famd_gbt_ehr_claims.out', 'a'))

scores(co_validation_low_FAMD, out_validation_comp_low)
