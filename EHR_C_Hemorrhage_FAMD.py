...
def scores(X,y):
    ...
    actual = y
    print(accuracy_score(actual,pred), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(f1_score(actual,pred), file = open('hemorrhage_famd_rf_ehr_claims.out', 'a'))
    print(fbeta_score(actual,pred, average = 'macro', beta = 2),file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(roc_auc_score(actual, best_clf.predict_proba(X)[:,1]), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(log_loss(actual,best_clf.predict_proba(X)[:,1]), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
...
def cross_val(X,y):
    ...
    print(np.mean(accuracy), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(np.mean(f1), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(np.mean(f2), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(np.mean(auc), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
    print(np.mean(log_loss), file = open('hemorrhage_famd_rf_ehr.out', 'a'))
...
print("gpop Sep 2",file = open('hemorrhage_famd_rf_ehr.out', 'a'))
#best_clf = rf(co_train_gpop_FAMD, out_train_hemorrhage_gpop)

cross_val(co_train_gpop_FAMD, out_train_hemorrhage_gpop)

print("",file = open('hemorrhage_famd_rf_ehr.out', 'a'))
...
#best_clf = rf(co_train_low_FAMD, out_train_hemorrhage_low)
...
#best_clf = rf(co_train_high_FAMD, out_train_hemorrhage_high)
